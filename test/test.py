...
async def test_project(dut):
    ...
    uart_sink = UartSink(dut.uart_tx, baud=960, bits=8)

    await ClockCycles(dut.clk, 15000) 
    assert dut.uo_out.value[7:4] == TEST_VALUE, f"Expected uo_out to be {TEST_VALUE}, got {dut.uo_out.value}"
    ...
    uart_source = UartSource(dut.uart_rx, baud=960, bits=8)
    txData = b"TEST"
